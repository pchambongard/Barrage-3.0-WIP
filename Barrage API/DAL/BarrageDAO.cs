...
namespace Barrage_API.DAL
{
	public class BarrageDAO
	{
		...
		public static async Task<List<Barrage>> GetBySecteurAsync(int idsecteur)
		{
			try
			{
				await using NpgsqlConnection connection = new("Server=srw-pgtest;Port=5432;Database=barrage;User Id=pgbarrage;Password=DbaBarrage!30;Pooling=false");
				...
				await using NpgsqlCommand commande = new()
				{
					Connection = connection,
					CommandText = "SELECT id, nom, id_type_barrage, usage, hauteur, longueur, " +
					"volume_retenu, surface_retenue, surface_bassin, latitude, longitude, altitude FROM barrages b WHERE b.id_secteur = :idsecteur;"
				};

				commande.Parameters.Add(new NpgsqlParameter<int>(":idsecteur", idsecteur));
		...
		public static async Task<int> StoreAsync(Barrage barrage, int idsecteur)
		{
			try
			{
				await using NpgsqlConnection connection = new("Server=srw-pgtest;Port=5432;Database=barrage;User Id=pgbarrage;Password=DbaBarrage!30;Pooling=false");
				...
				if (barrage.IsNew())
				{
					...
					{
						Connection = connection,
						CommandText = "INSERT INTO barrages(nom, id_type_barrage, usage, id_secteur, hauteur, longueur, volume_retenu, surface_retenue, surface_bassin, latitude, longitude, altitude) " +
							"values(:nom, :type, :usage, :id_secteur, :hauteur, :longueur, :volume_retenu, :surface_retenue, :surface_bassin, :latitude, :longitude, :altitude) returning id;"
					};
					...
					commande2.Parameters.Add(new NpgsqlParameter<int>(":id_secteur", idsecteur));
				...
				else
				{
					...
					{
						Connection = connection,
						CommandText = "UPDATE barrages SET nom=:nom, id_type_barrage=:type, usage=:usage, id_secteur=:id_secteur, hauteur=:hauteur, longueur=:longueur, " +
